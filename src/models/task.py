...
class ResearchTask(Base):
    ...
    stage: Mapped[TaskStage] = mapped_column(
        SQLEnum(TaskStage), default=TaskStage.planning
    )
    ...
    progress_subtasks_total: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False
    )
    progress_subtasks_completed: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False
    )
    progress_papers_collected: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False
    )
    progress_findings_generated: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False
    )

    graph_state_snapshot_ref: Mapped[str | None] = mapped_column(
